...
type Options = ConcatOptions | SliceOptions | SelectOptions | JoinOptions | FilterOptions;
...
function printUsage() {
  console.log("Usage: node tsv.ts <command> [options]");
  console.log("");
  console.log("Commands:");
  console.log("  concat    Concatenate multiple TSV files");
  console.log("  slice     Extract rows from TSV file");
  console.log("  select    Select specific columns from TSV file");
  console.log("  join      Inner join two TSV files");
  console.log("  filter    Filter rows based on column conditions");
  console.log("");
  console.log("Options:");
  console.log("  -h, --help            Show this help message");
  console.log("");
  console.log('Use "node tsv.ts <command> --help" for more information about a command.');
}

function printConcatUsage() {
  console.log("Usage: node tsv.ts concat <input_files>... [options]");
  console.log("");
  console.log(
    "Concatenate multiple TSV files into one, preserving the header from the first file."
  );
  console.log("");
  console.log("Arguments:");
  console.log("  input_files           One or more input TSV files to concatenate");
  console.log("");
  console.log("Options:");
  console.log("  --output <file.tsv>   Output file (default: stdout)");
  console.log("  -o <file.tsv>         Short form of --output");
  console.log("  -h, --help            Show this help message");
  console.log("");
  console.log("Examples:");
  console.log("  node tsv.ts concat file1.tsv file2.tsv file3.tsv --output merged.tsv");
  console.log("  node tsv.ts concat file1.tsv file2.tsv -o merged.tsv");
  console.log("  node tsv.ts concat file1.tsv file2.tsv > merged.tsv");
}

function printSliceUsage() {
  console.log("Usage: node tsv.ts slice <input_file.tsv> <start> <end> [options]");
  console.log("");
  console.log("Extract a range of data rows from a TSV file, preserving the header.");
  console.log("");
  console.log("Arguments:");
  console.log("  input_file.tsv        Path to input TSV file");
  ...
  console.log("");
  console.log("Examples:");
  console.log("  node tsv.ts slice input.tsv 100 1000 --output subset.tsv");
  console.log("  node tsv.ts slice input.tsv 0 500 -o first500.tsv");
  console.log("  node tsv.ts slice input.tsv 100 1000 > subset.tsv");
}

function printSelectUsage() {
  console.log("Usage: node tsv.ts select <input_file.tsv> <column_indices> [options]");
  console.log("");
  console.log("Select specific columns from a TSV file by their indices.");
  console.log("");
  console.log("Arguments:");
  console.log("  input_file.tsv        Path to input TSV file");
  ...
  console.log("");
  console.log("Examples:");
  console.log("  node tsv.ts select input.tsv 0,1,3 --output selected.tsv");
  console.log("  node tsv.ts select input.tsv 2,5,7 -o columns.tsv");
  console.log("  node tsv.ts select input.tsv 0,1,3 > selected.tsv");
}

function printJoinUsage() {
  console.log(
    "Usage: node tsv.ts join <left_file.tsv> <right_file.tsv> <left_column> <right_column> [options]"
  );
  console.log("");
  console.log("Inner join two TSV files on specified columns.");
  console.log("");
  console.log("Arguments:");
  console.log("  left_file.tsv         Path to left TSV file");
  console.log("  right_file.tsv        Path to right TSV file");
  ...
  console.log("");
  console.log("Examples:");
  console.log("  node tsv.ts join users.tsv orders.tsv 0 1 --output joined.tsv");
  console.log("  node tsv.ts join left.tsv right.tsv 2 0 -o result.tsv");
  console.log("  node tsv.ts join file1.tsv file2.tsv 1 1 > joined.tsv");
  console.log("");
  console.log(
...
function printFilterUsage() {
  console.log("Usage: node tsv.ts filter <input_file.tsv> <condition> [options]");
  console.log("");
  console.log("Filter rows from a TSV file based on column conditions.");
  console.log("");
  console.log("Arguments:");
  console.log("  input_file.tsv        Path to input TSV file");
  console.log("  condition             Filter condition in format: .<idx>=value or .<idx>!=value");
  console.log("                        where idx is 0-based column index");
  console.log("");
  console.log("Options:");
  console.log("  --output <file.tsv>   Output file (default: stdout)");
  console.log("  -o <file.tsv>         Short form of --output");
  ...
  console.log("");
  console.log("Examples:");
  console.log('  node tsv.ts filter input.tsv ".1=10" --output filtered.tsv');
  console.log('  node tsv.ts filter input.tsv ".0!=empty" -o result.tsv');
  console.log('  node tsv.ts filter input.tsv ".2 = active" > filtered.tsv');
  console.log("");
  console.log("Note: Spaces around = or != operators are allowed.");
}
...
function parseArguments(): Options {
  ...
  if (command === "filter") {
    ...
    if (positionals.length !== 3) {
      console.error("Error: filter command requires exactly 2 arguments: <file> <condition>");
      printFilterUsage();
      process.exit(1);
    }

    const file = positionals[1];
    const condition = positionals[2];
    ...
    // Parse and validate condition format
    const conditionMatch = condition.match(/^\.(\d+)\s*(=|!=)\s*(.*)$/);
    if (!conditionMatch) {
      console.error(
        "Error: Invalid condition format. Expected: .<column_index>=value or .<column_index>!=value"
      );
      printFilterUsage();
      process.exit(1);
    }

    return { command: "filter", file, condition, output };
  }
...
async function concatTSV(options: ConcatOptions): Promise<void> {
  ...
  if (options.output) {
    ...
    console.log(`Concatenated ${options.files.length} TSV files to "${outputTarget}"`);
...
async function sliceTSV(options: SliceOptions): Promise<void> {
  const outputTarget = options.output || "stdout";
...
async function selectTSV(options: SelectOptions): Promise<void> {
  const outputTarget = options.output || "stdout";
  ...
  for await (const line of rl) {
    // Split the TSV line (simple comma splitting - doesn't handle quoted commas)
...
async function joinTSV(options: JoinOptions): Promise<void> {
  const outputTarget = options.output || "stdout";
...
async function filterTSV(options: FilterOptions): Promise<void> {
  const outputTarget = options.output || "stdout";

  // Parse condition
  const conditionMatch = options.condition.match(/^\.(\d+)\s*(=|!=)\s*(.*)$/);
  if (!conditionMatch) {
    console.error("Error: Invalid condition format");
    process.exit(1);
  }

  const columnIndex = parseInt(conditionMatch[1]);
  const operator = conditionMatch[2] as "=" | "!=";
  const value = conditionMatch[3];

  if (options.output) {
    console.log(
      `Filtering "${options.file}" where column ${columnIndex} ${operator} "${value}"...`
    );
  }
  ...
  for await (const line of rl) {
    ...
    if (currentLine === 0) {
      ...
      if (columnIndex >= fields.length) {
        console.error(
          `Error: Column index ${columnIndex} is out of range. File has only ${
            fields.length
          } columns (0-${fields.length - 1}).`
        );
        process.exit(1);
      ...
      const fieldValue = fields[columnIndex] || "";
      let shouldInclude = false;

      if (operator === "=") {
        shouldInclude = fieldValue === value;
      } else if (operator === "!=") {
        shouldInclude = fieldValue !== value;
      }
  ...
  if (options.output) {
    output.end();
    console.log(`Filtered ${filteredRows} rows from ${currentLine - 1} total data rows.`);
    console.log(`Output written to "${outputTarget}".`);
  }
}
...
if (process.argv[1] === import.meta.filename) {
  ...
  try {
    if (options.command === "concat") {
      await concatTSV(options);
    } else if (options.command === "slice") {
      await sliceTSV(options);
    } else if (options.command === "select") {
      await selectTSV(options);
    } else if (options.command === "join") {
      await joinTSV(options);
    } else if (options.command === "filter") {
      await filterTSV(options);
    }
  } catch (error) {
